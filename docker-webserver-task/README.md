...
# Run the container
docker run -d -p 9000:80 --name webserver-task my-webserver
```
Then open `http://localhost:9000` in your browser to view the custom page.
...
```bash
docker exec -it webserver-task rm /usr/share/nginx/html/index.html
```
This caused the site to return a 403/404 error in the browser. The issue was diagnosed using:
```bash
docker logs webserver-task
docker exec -it webserver-task sh
```
